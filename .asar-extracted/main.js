...
const ttsLogPath = path.join(configDir, "tts-debug.log");
function ttsLog(msg) {
  try {
    const ts = new Date().toISOString().replace("T", " ").substring(0, 19);
    fs.appendFileSync(ttsLogPath, "[" + ts + "] " + msg + "\n", "utf-8");
  } catch(e) {}
}
let mainWin;
let tray = null;
...
const createMainWin = () => {
  ...
  ipcMain.handle('tts-write-log', async (event, message) => {
    ttsLog(message);
    return true;
  });
  ipcMain.handle("generate-tts", async (event, voiceConfig) => {
    let { text, speed, plugin, config } = voiceConfig;
    ttsLog("[Main] generate-tts called: voiceName=" + (config?.voiceName || "none") + ", engine=" + (plugin?.key || "none"));
    // 豆包 TTS 引擎
    if (plugin && plugin.key === "doubao-tts") {
      try {
        ...
        ttsLog("[Main] doubao-tts succeeded");
        return pcmData;
      } catch (e) {
        console.error("[DoubaoTTS] generate error:", e);
        ttsLog("[Main] doubao-tts ERROR: " + (e.message || e));
        return null;
      }
    }
    let voiceFunc = plugin.script;
    ttsLog("[Main] Evaluating plugin script, script length=" + voiceFunc.length);
    // eslint-disable-next-line no-eval
    eval(voiceFunc);
    ttsLog("[Main] Plugin script evaluated, getAudioPath exists: " + (typeof global.getAudioPath === 'function'));
    try {
      ttsLog("[Main] Calling getAudioPath...");
      const result = await global.getAudioPath(text, speed, dirPath, config);
      ttsLog("[Main] getAudioPath returned, path=" + result);
      return result;
    } catch (e) {
      log.error(e);
      ttsLog("[Main] generate-tts ERROR: " + (e.message || e));
      return null;
    }
  });
