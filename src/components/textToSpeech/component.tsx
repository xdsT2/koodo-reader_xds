...
import TTSUtil, { ttsLog } from "../../utils/reader/ttsUtil";
...
> {
  ...
  // 朗读代次：每次 handleStartAudio 递增，让旧的 handleCustomRead 循环知道自己已过期
  ...
  handleSpeechAutoStartRequest = async () => {
    ...
    // 重置 TTSUtil.isPaused —— handleStop → stopAudio 会设为 true，
    // 但新会话的 cacheAudio 要从干净状态开始（和 handleVoiceSwitch 同类问题）
    TTSUtil.isPaused = false;
  ...
  handleStartAudio = async () => {
    // 递增代次，让旧的 handleCustomRead 循环知道自己已过期
    this.readGeneration++;
    ttsLog("[TTS] handleStartAudio: engine=" + ConfigService.getReaderConfig("voiceEngine") + ", isAuthed=" + this.props.isAuthed + ", gen=" + this.readGeneration);
    if (
      this.props.isAuthed &&
      ...
      ttsLog("[TTS] handleStartAudio: fetching user info...");
      await fetchUserInfo();
      ttsLog("[TTS] handleStartAudio: user info fetched");
    }
    if (
      ConfigService.getReaderConfig("voiceEngine") ===
      ...
      ttsLog("[TTS] handleStartAudio: auto-switching to system voice (not authed for AI voice)");
      ConfigService.setReaderConfig("voiceEngine", "system");
    }
  ...
  handlePrevSentence = async () => {
    ...
    this.readGeneration++; // 让旧的 handleCustomRead 循环退出
  ...
  handleVoiceSwitch = async (
    ...
    this.setState({ isPaused: true }, () => {
      this.setState({ isPaused: false }, () => {
        // 必须显式重置 TTSUtil 的静态暂停标志
        // 上面的 setState 只改组件状态，TTSUtil.isPaused 仍为 true 会导致 cacheAudio 直接 break
        TTSUtil.isPaused = false;
        // 递增代次，让旧的 handleCustomRead 循环退出
        this.readGeneration++;
  ...
  handleAudio = async () => {
    ttsLog("[TTS] handleAudio: start");
    this.nodeList = await this.handleGetText();
    ttsLog("[TTS] handleAudio: got " + this.nodeList.length + " nodes");
    if (this.nodeList.length === 0) {
      ttsLog("[TTS] handleAudio: nodeList is empty, returning");
      return;
    }
    ttsLog("[TTS] handleAudio: first node engine=" + this.nodeList[0].voiceEngine + ", voice=" + this.nodeList[0].voiceName);
  ...
  async handleCustomRead(nodeIndex: number) {
    const myGen = this.readGeneration;
    ttsLog("[TTS] handleCustomRead: start from index=" + nodeIndex + ", total=" + this.nodeList.length + ", gen=" + myGen);
    ...
    for (let index = nodeIndex; index < this.nodeList.length; index++) {
      // 代次变更 → 说明用户重新开始了新的朗读，旧循环应立即退出
      if (this.readGeneration !== myGen) {
        ttsLog("[TTS] handleCustomRead: generation changed at index=" + index + ", aborting old loop");
        return;
      }
      if (this.state.isPaused || !this.state.isAudioOn) {
        ttsLog("[TTS] handleCustomRead: paused/stopped at index=" + index + ", isPaused=" + this.state.isPaused + ", isAudioOn=" + this.state.isAudioOn);
        return;
      }
      ...
      if (index === nodeIndex) {
        ttsLog("[TTS] handleCustomRead: first run, caching index=" + index + "...");
        ...
        ttsLog("[TTS] handleCustomRead: first cacheAudio done, result=" + result);
        toast.dismiss("tts-load");
        // 代次变更 → 新的朗读已开始，旧循环退出（不报错、不 setState）
        if (this.readGeneration !== myGen) {
          ttsLog("[TTS] handleCustomRead: generation changed after cacheAudio, aborting");
          return;
        }
        if (result === "error") {
          ttsLog("[TTS] handleCustomRead: cacheAudio error, stopping");
          toast.error(this.props.t("Audio loading failed, stopped playback"));
          this.setState({ isAudioOn: false });
          this.nodeList = [];
          return;
      ...
      if (this.nodeList[index].voiceEngine === "system") {
        ttsLog("[TTS] handleCustomRead: switching to system voice at index=" + index);
        await this.handleSystemRead(index);
        break;
      }

      ttsLog("[TTS] handleCustomRead: pre-caching next, index=" + (index + 1));
      TTSUtil.cacheAudio(
        index + 1,
        speed * 100 - 100,
        this.props.plugins,
  ...
  handleSpeech = async (index: number) => {
    ttsLog("[TTS] handleSpeech: index=" + index);
    return new Promise<string>(async (resolve) => {
      let res = await TTSUtil.readAloud(index);
      ttsLog("[TTS] handleSpeech: readAloud returned=" + res + " for index=" + index);
      if (res === "loaderror") {
        ttsLog("[TTS] handleSpeech: loaderror for index=" + index + ", audioPaths=" + JSON.stringify(TTSUtil.getAudioPaths().map(p => p.index)));
        resolve("error");
      } else {
        let player = TTSUtil.getPlayer();
        player.on("end", async () => {
          if (!(this.state.isAudioOn && this.props.isReading)) {
            ttsLog("[TTS] handleSpeech: onend but not reading anymore, resolve end");
            resolve("end");
          }
          ttsLog("[TTS] handleSpeech: onend, resolve start for index=" + index);
          resolve("start");
        });
      }
    });
