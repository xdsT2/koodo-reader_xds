  ...
  return new Promise<Record<string, any> | false>((resolve) => {
    ...
    const inputHtml = keys
      .map((key) => {
        ...
        const displayLabel = labels?.[key] ?? label ?? key;
        const descriptionHtml = description
          ? `<div style="font-size:12px;color:#666;margin-top:2px;margin-bottom:4px">${escapeAttr(description)}</div>`
          : "";
        let inputHtml = "";
        if (inputType === "select" && options.length > 0) {
          const optionsHtml = options
            .map(
              (opt) =>
                `<option value="${escapeAttr(opt.value)}"${opt.value === value ? " selected" : ""}>${escapeAttr(opt.label)}</option>`
            )
            .join("");
          inputHtml = `<select name="${key}" style="width:100%;padding:8px;border:1px solid #ddd;border-radius:4px;font-size:14px" required>${optionsHtml}</select>`;
        } else {
          inputHtml = `<input name="${key}" type="${escapeAttr(inputType)}" placeholder="${escapeAttr(placeholder)}" value="${escapeAttr(value)}" style="width:100%;padding:8px;border:1px solid #ddd;border-radius:4px;font-size:14px" required />`;
        }
        return [
          `<div style="margin-bottom:10px">`,
          `<label style="display:block;margin-bottom:4px;font-weight:500">${escapeAttr(displayLabel)}</label>`,
          descriptionHtml,
          inputHtml,
          `</div>`,
        ].join("");
